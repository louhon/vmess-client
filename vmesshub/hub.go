...
import (
	...
	"github.com/louhon/vmess.cli/vmess"

	"github.com/louhon/vmess.cli/socks5"

	"github.com/louhon/vmess.cli/ext"
)
...
func (vh *VmessHub) handleSocks5Data(client net.Conn) (err error) {
	...
	if ver != 5 {
		log.Printf("ver should be 5, got %v\n", ver)
		return
	}

	// 1: connect 2: bind
	if cmd != 1 && cmd != 2 {
		log.Printf("bad cmd:%v\n", cmd)
		return
	}
